...
const JobListing = () => {
    ...
    useEffect(()=>{
        ...
        const matchedSearchLoction = job => searchFilter.location === "" || job.location.toLowerCase().includes(searchFilter.location.toLowerCase())

        const newFilterJobs = jobs.slice().reverse().filter(
            job => matchedCategory(job) && matchedLocation(job) && matchedTitle(job) && matchedSearchLoction(job)
        )
  ...
  return (
    <div className='container 2xl:px-20 mx-auto flex flex-coll lg:flex-row max-lg:space-y-8 py-8'>
        {/* //sidebar */}

        <div className='w-full lg:w-1/4 bg-white px-4'>
            {/* search filtr from hero component */}
            {
